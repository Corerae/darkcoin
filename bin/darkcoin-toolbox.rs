use darkcoin::{mdbx_table_sizes, stagedsync};
use std::path::PathBuf;
use structopt::StructOpt;

#[derive(StructOpt)]
#[structopt(name = "Darkcoin Toolbox", about = "Utilities for Darkcoin Ethereum client")]
pub enum Opt {
...
async fn blockhashes(chaindata: PathBuf) -> anyhow::Result<()> {
    let env = darkcoin::MdbxEnvironment::<mdbx::NoWriteMap>::open_ro(
        mdbx::Environment::new(),
        &chaindata,
        &darkcoin::kv::tables::TABLE_MAP,
    )?;
